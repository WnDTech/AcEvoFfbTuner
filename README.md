...
| **Logitech** | `LogitechTrueForceProvider` (TrueForce stream) | **Active** | RS50/G PRO via the native TrueForce stream (500 Hz, no G HUB, no PC-mode dependency); the stream survives game-source switches and reconnects without re-init, delivers force while the game holds the wheel exclusively, and sits in silent standby when idle (normal game FFB untouched); G29/G920/G923 via DirectInput constant force + vibration |
