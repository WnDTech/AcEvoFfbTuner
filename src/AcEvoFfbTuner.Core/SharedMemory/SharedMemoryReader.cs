...
public sealed class SharedMemoryReader : ISharedMemoryReader
{
    ...
    private void DumpPhysicsBytes(byte[] buffer, SPageFilePhysicsEvo physics)
    {
        try
        {
            ...
            sb.AppendLine($"Offset   608: Fx            = FL:{BitConverter.ToSingle(buffer, 608):F4} FL:{BitConverter.ToSingle(buffer, 612):F4} RL:{BitConverter.ToSingle(buffer, 616):F4} RR:{BitConverter.ToSingle(buffer, 620):F4}");
            ...
            sb.AppendLine($"Offset   784: KerbVibration = {BitConverter.ToSingle(buffer, 784):F4}");
            sb.AppendLine($"Offset   788: SlipVibrations= {BitConverter.ToSingle(buffer, 788):F4}");
            sb.AppendLine($"Offset   792: RoadVibrations= {BitConverter.ToSingle(buffer, 792):F4}");
            sb.AppendLine($"Offset   796: AbsVibrations = {BitConverter.ToSingle(buffer, 796):F4}");
            sb.AppendLine($"");
