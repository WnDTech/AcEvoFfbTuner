...
public sealed class LiveAutoTuner
{
    ...
    private const float ForceFloor = 0.15f;
    ...
    private const int TickWindowSize = 20;
    private const int MinCorrectionsToSlowDown = 8;
        ...
        float speedKmh, float centerForceRatio, float avgCenterForce, float avgSteerVel)
    {
        ...
        float cooldownSeconds = CorrectionsApplied > MinCorrectionsToSlowDown ? 20f : 3f;
        if ((now - _lastCorrectionTime).TotalSeconds < cooldownSeconds) return;

        if (centerForceRatio > 0.30f && avgCenterForce > 0.02f && _centerTicks > 8)
        {
        ...
        if (clipRate > 0.05f)
        {
            ...
            float reducedGain = currentGain * 0.88f;
            ...
            float increasedComp = Math.Min(currentComp + 0.08f, CompPowerCeiling);
            _pipeline.CompressionPower = increasedComp;
            LogCorrection("CompressionPower", currentComp, increasedComp, $"Clipping compress");
            return;
        }

        if (avgForce > 0.25f && peakForce > 0.40f)
        {
            float aggressiveness = Math.Clamp((avgForce - 0.25f) / 0.15f, 0f, 1f);
            float factor = 0.96f - aggressiveness * 0.06f;
        ...
        if (maxDelta > 0.06f && speedKmh > 30f)
        {
            float currentFriction = _pipeline!.Damping.FrictionLevel;
            float increasedFriction = Math.Min(currentFriction * 1.12f, FrictionCeiling);
    ...
    private void CorrectCenterOscillation(float centerForceRatio, float avgCenterForce, float speedKmh)
    {
        ...
        float increasedCenterSupp = Math.Min(currentCenterSupp + 1.0f, CenterSuppCeiling);
        ...
        float currentNoise = _pipeline!.NoiseFloor;
        float increasedNoise = Math.Min(currentNoise + 0.002f, NoiseFloorCeiling);
        _pipeline.NoiseFloor = increasedNoise;
        LogCorrection("NoiseFloor", currentNoise, increasedNoise, $"Center osc noise");

        float currentHyst = _pipeline!.HysteresisThreshold;
        float increasedHyst = Math.Min(currentHyst + 0.003f, HystCeiling);
    ...
    public void OnLapSummary(DiagnosticLapSummary summary)
    {
        ...
        var now = DateTime.UtcNow;
        if ((now - _lastCorrectionTime).TotalSeconds < 10) return;

        if (summary.TotalOscillations >= 1)
        {
            float currentDamp = _pipeline!.Damping.SpeedDampingCoefficient;
            float increasedDamp = Math.Min(currentDamp * 1.15f, SpeedDampCeiling);
            _pipeline.Damping.SpeedDampingCoefficient = increasedDamp;
        ...
        if (summary.SuspiciousRoadVibrationSnaps >= 1)
        {
            float current = _pipeline!.VibrationMixer.SuspensionRoadGain;
            float reduced = current * 0.90f;
        ...
        {
            float currentGain = _pipeline!.OutputGain;
            float reducedGain = currentGain * 0.90f;
            if (reducedGain >= ForceFloor)
            {
                _pipeline.OutputGain = reducedGain;
            ...
            float currentComp = _pipeline.CompressionPower;
            float increasedComp = Math.Min(currentComp + 0.1f, CompPowerCeiling);
            _pipeline.CompressionPower = increasedComp;
